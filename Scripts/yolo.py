...
宋进雨
YoLov8预测模型
二次开发，调用detect_image()函数，返回类名，置信度，区域
"""
...
class YOLO(object):
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        # ---------------------------------------------------------#
        if count:
            # top_label为每个框对应的类别标签在voc_class里的位置
            ...
            for i in range(self.num_classes):
                ...
                classes_nums[i] = num
            # class_nums为voc_class中每个类名标签对应检测框的数量
        ...
        #   图像绘制
        # ---------------------------------------------------------#
        score_pexil_pos = np.empty((len(top_label), 4), np.float32)
        classes_name = []
        for i, c in list(enumerate(top_label)):
            ...
            # label_size = draw.textsize(label, font) 在pillow新的版本中已经不再使用，改为如下
            bbx = draw.textbbox((0, 0), label, font=font)
            label_size = (bbx[2] - bbx[0], bbx[3] - bbx[1])
            label = label.encode('utf-8')
            score_pexil_pos[i, :] = np.array([left, right, top, bottom])
            classes_name.append(predicted_class)
            ...
            del draw
        # print('检测到的目标类别名:\n', classes_name)
        image.save("img/result/target_show.jpg")
        return classes_name, score_pexil_pos
